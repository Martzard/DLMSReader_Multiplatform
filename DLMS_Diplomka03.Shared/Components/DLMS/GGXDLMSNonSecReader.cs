...
namespace DLMS_Diplomka03.Shared.Components.DLMS;

public class GXDLMSNonSecReader(GXDLMSClient nonSecClient, IGXMedia media, TraceLevel trace, string invocationCounter)
    {
        //ContentPage ContentPage { get; set; }
        ...
        IGXMedia Media = media;
        TraceLevel Trace = trace;
        GXDLMSClient Client = nonSecClient;
        // Invocation counter (frame counter).
        string InvocationCounter = invocationCounter;
        ...
        public GXDLMSObjectCollection ReadAll(string outputFile)
        {
            try
            {
                //UiLogger.Log("Reading all objects!");
        ...
        public GXDLMSObject ReadSingleObject(GXDLMSObject selectedObject)
        {
            ...
            if (obj == null)
            {
                ...
                //{
                //    Console.WriteLine("Objekt s LN " + selectedObject.LogicalName + " nebyl nalezen v asociaci měřiče.");
                //    return;
                //});
                Console.WriteLine("Objekt s LN " + selectedObject.LogicalName + " nebyl nalezen v asociaci měřiče.");
            }
            ...
            else if (obj is IGXDLMSBase baseObj)
            {
                foreach (int attributeIndex in baseObj.GetAttributeIndexToRead(true))
                {
                    ...
                    {
                        Console.WriteLine($"Chyba při čtení atributu {attributeIndex} objektu {selectedObject.LogicalName}: {ex.Message}");
                    }
                }
            }
            return obj;
        ...
        public object Read(GXDLMSObject it, int attributeIndex)
        {
            ...
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Can't read " + it.ToString() + ". Not enought acccess rights."); });
        ...
        public void AarqRequest()
        {
            ...
            if (aarq.Length != 0)
            {
                ...
                if (Trace > TraceLevel.Info)
                {
                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Parsing AARE reply succeeded."); });
        ...
        void InitializeOpticalHead()
        {
            ...
            if (Trace > TraceLevel.Info)
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("IEC Sending:" + data); });
            ...
            lock (Media.Synchronous)
            {
                ...
                if (!Media.Receive(p))
                {
                    ...
                    if (Trace > TraceLevel.Info)
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(str); });
                ...
                if (p.Reply == data)
                {
                    ...
                    if (!Media.Receive(p))
                    {
                        ...
                        if (Trace > TraceLevel.Info)
                        {
                            //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(data); });
            ...
            if (Trace > TraceLevel.Info)
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("IEC received: " + p.Reply); });
            ...
            if (Trace > TraceLevel.Info)
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(DateTime.Now.ToLongTimeString() + "\tBaudRate is : " + BaudRate.ToString()); });
            ...
            if (Trace > TraceLevel.Info)
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(DateTime.Now.ToLongTimeString() + "\tMoving to mode E.", arr); });
            ...
            lock (Media.Synchronous)
            {
                ...
                if (p.Reply != null)
                {
                    if (Trace > TraceLevel.Info)
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Received: " + p.Reply); });
        ...
        {
            //UpdateEditor(("Standard: " + Client.Standard).ToString());
            //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Standard: " + Client.Standard); });
            ...
            if (!Client.PreEstablishedConnection)
            {
                ...
                if (Trace > TraceLevel.Info)
                {
                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Parsing AARE reply" + reply.ToString()); });
                    Console.WriteLine("Parsing AARE reply" + reply.ToString());
                }
                //Parse reply.
                Client.ParseAAREResponse(reply.Data);
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Conformance: " + Client.NegotiatedConformance); });
        ...
        public void GetScalersAndUnits()
        {
            ...
            {
                //UpdateEditor(("Read scalers and units from the device.").ToString());
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Read scalers and units from the device."); });
            ...
            if ((Client.NegotiatedConformance & Gurux.DLMS.Enums.Conformance.MultipleReferences) == 0)
            {
                ...
                foreach (GXDLMSObject it in objs)
                {
                    try
                    {
                        ...
                        {
                            //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(it.Name); });
        ...
        public void GetProfileGenericColumns()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                try
                {
                    ...
                    if (Trace > TraceLevel.Warning)
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(it.LogicalName); });
                    ...
                    if (Trace > TraceLevel.Warning)
                    {
                        ...
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(sb.ToString()); });
        ...
        public void ShowValue(object val, int pos)
        {
            ...
            if (Trace > TraceLevel.Warning)
            {
                ...
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Index: " + pos + " Value: " + val); });
        ...
        public void GetProfileGenerics()
        {
            ...
            foreach (GXDLMSObject it in Client.Objects.GetObjects(ObjectType.ProfileGeneric))
            {
                ...
                if (Trace > TraceLevel.Warning)
                {
                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("-------- Reading " + it.GetType().Name + " " + it.Name + " " + it.Description); });
                ...
                if (Trace > TraceLevel.Warning)
                {
                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Entries: " + entriesInUse + "/" + entries); });
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Error! Failed to read first row: " + ex.Message); });
                ...
                if ((Client.NegotiatedConformance & (Gurux.DLMS.Enums.Conformance.ParameterizedAccess | Gurux.DLMS.Enums.Conformance.SelectiveAccess)) != 0)
                {
                    ...
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Error! Failed to read last day: " + ex.Message); });
        ...
        public void GetReadOut()
        {
            foreach (GXDLMSObject it in Client.Objects)
            {
                ...
                if (!(it is IGXDLMSBase))
                {
                    ...
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Unknown Interface: " + it.ObjectType.ToString()); });
                ...
                foreach (int pos in (it as IGXDLMSBase).GetAttributeIndexToRead(true))
                {
                    ...
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Error! " + it.GetType().Name + " " + it.Name + "Index: " + pos + " " + ex.Message); });
                        Console.WriteLine("Error! " + it.GetType().Name + " " + it.Name + "Index: " + pos + " " + ex.Message);
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(ex.ToString()); });
                        ...
                        if (notify.IsComplete && notify.Data.Data != null)
                        {
                            ...
                            if (!notify.IsMoreData)
                            {
                                if (notify.PrimeDc != null)
                                {
                                    ...
                                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(notify.PrimeDc); });
                                ...
                                else
                                {
                                    ...
                                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(xml); });
        ...
        public void Disconnect()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    if (Trace > TraceLevel.Info)
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Disconnecting from the meter."); });
        ...
        public void Release()
        {
            if (Media != null && Client != null)
            {
                try
                {
                    if (Trace > TraceLevel.Info)
                    {
                        //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Release from the meter."); });
                ...
                {
                    //All meters don't support Release.
                    //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine("Release failed. " + ex.Message); });
        ...
        void WriteTrace(string line)
        {
            if (Trace > TraceLevel.Info)
            {
                //MainThread.BeginInvokeOnMainThread(() => { Console.WriteLine(line); });
